...
import { FaFileInvoiceDollar, FaRegPlusSquare } from "react-icons/fa";
...
function reviewPayables() {
  ...
  const columns = [
    ...
    { name: 'Total Amount', selector: row => formatCurrency(row.totalRequest), width: '180px' },
    {
      ...
      selector: row => (
        <span style={{
          color: row.status === 'Pending' ? 'red' : 'blue',
          fontWeight: 'bold'
        }}>
          {row.status}
        </span>
  ...
  const onProcessColumns = [
    ...
    { name: 'Total Amount', selector: row => formatCurrency(row.totalRequest), width: '150px' },
    {
      ...
      selector: row => (
        <span style={{
          color: row.status === 'On process' ? 'blue' : 'red',
          fontWeight: 'bold'
        }}>
          {row.status}
        </span>
  ...
  const handleOnProcessSearch = (event) => setOnProcessSearchText(event.target.value);
          ...
          <div className="mx-4">
            <div className="overflow-x-auto w-full">
              <DataTable
                ...
                highlightOnHover
                pointerOnHover
                onRowClicked={handlePendingRowClick}
                subHeader
                subHeaderComponent={
                  <input
                    type="text"
          ...
          <div className="modal-box w-full h-full max-w-[1600px]">
            <div className='bg-white p-10'>
              ...
              <div className="space-y-4">
                ...
                <iframe src={selectedRowOnProcessData.documents} width="100%" height="600px" title="PDF Viewer" />
                <div className="flex justify-between">
                  <p className="font-medium"><strong>Reason:</strong></p>
                  <p className="text-gray-700">{selectedRowOnProcessData.reason}</p>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <p className={`text-blue-700 font-bold ${selectedRowOnProcessData?.status}`}>{selectedRowOnProcessData?.status}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="flex gap-4 w-full">
                ...
                <div className="w-full">
                  ...
                  <input
                    ...
                    type="Number"
                    value={totalRequest}
                    onChange={(e) => setTotalRequest(e.target.value)}
                    required
                  />
                </div>
              </div>
